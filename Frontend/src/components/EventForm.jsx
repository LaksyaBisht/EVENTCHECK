...
const EventForm = ({ onSubmit, loading = false }) => {
    ...
    const handleGenerateDescription = async () => {
        ...
        const prompt = formData.event_description;

        if (!eventName) {
            setErrors(prev => ({ ...prev, event_name: "Please enter an event name before generating a description." }));
            return;
        }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        placeholder="Give a detailed prompt to get the desired description..."
                    />
                    <button
                        type="button"
                        onClick={handleGenerateDescription}
                        disabled={isGenerating}
                        className={`mt-2 w-full flex items-center justify-center space-x-2 text-white font-semibold py-2 px-4 rounded-lg transition-colors duration-200
                        ${isGenerating ? 'bg-indigo-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                    >
                        {isGenerating ? (
                            <>
                                <div className="loading-spinner h-5 w-5 mr-2 text-white"></div>
